...
export function ResultsDisplay({ result, showEquivalentPerformance = true, liftFamily, variant }: ResultsDisplayProps) {
  ...
  const [timeInput, setTimeInput] = useState<string>(defaultTime.toFixed(1));
  const parsedTime = parseFloat(timeInput);
  const time = Number.isFinite(parsedTime) && parsedTime > 0 ? parsedTime : defaultTime;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg shadow-sm border border-slate-700 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-blue-900/20 rounded-lg border border-blue-500/30">
            <div className="text-sm text-blue-200 mb-1">{lifterA.name}</div>
          ...
          <div className="text-center p-4 bg-orange-900/20 rounded-lg border border-orange-500/30">
            <div className="text-sm text-orange-200 mb-1">{lifterB.name}</div>
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-slate-300">Work & Power Output</h4>
              {/* Time Input */}
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-20 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-xs text-white font-medium focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  min="0.01"
                  step="0.01"
                />
                <span className="text-xs text-slate-500">s (avg: {(reps * 2.5).toFixed(1)}s)</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-blue-900/20 rounded-lg border border-blue-500/30">
                <div className="text-xs text-blue-300 mb-2 font-semibold">{lifterA.name}</div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-slate-400">Work/Rep</div>
                    <div className="text-lg font-bold text-blue-400">{lifterA.metrics.workPerRep.toFixed(0)} J</div>
                    <div className="text-xs text-slate-500 mt-0.5">{(lifterA.metrics.scoreP4P / (lifterA.metrics.totalWork / lifterA.metrics.workPerRep)).toFixed(1)} P4P/rep</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-400">Power/Rep</div>
                    <div className="text-lg font-bold text-blue-400">{(powerA / reps).toFixed(0)} W</div>
                ...
                <div className="mt-3 pt-3 border-t border-blue-500/30">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-xl font-bold text-blue-400">{lifterA.metrics.totalWork.toFixed(0)} J</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-slate-400">Total Power</div>
                      <div className="text-xl font-bold text-blue-400">{powerA.toFixed(0)} W</div>
                    </div>
                  </div>
                  <div className="text-xs text-slate-500 mt-1">{reps.toFixed(0)} reps in {time.toFixed(1)}s</div>
                </div>
              </div>

              {/* Lifter B Work & Power */}
              <div className="p-4 bg-orange-900/20 rounded-lg border border-orange-500/30">
                <div className="text-xs text-orange-300 mb-2 font-semibold">{lifterB.name}</div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-slate-400">Work/Rep</div>
                    <div className="text-lg font-bold text-orange-400">{lifterB.metrics?.workPerRep.toFixed(0) || "—"} J</div>
                    <div className="text-xs text-slate-500 mt-0.5">{lifterB.metrics ? (lifterB.metrics.scoreP4P / (lifterB.metrics.totalWork / lifterB.metrics.workPerRep)).toFixed(1) : "—"} P4P/rep</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-400">Power/Rep</div>
                    <div className="text-lg font-bold text-orange-400">{lifterB.metrics ? (powerB / reps).toFixed(0) : "—"} W</div>
                    <div className="text-xs text-slate-500 mt-0.5">&nbsp;</div>
                  </div>
                </div>
                <div className="mt-3 pt-3 border-t border-orange-500/30">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-xs text-slate-400">Total Work</div>
                      <div className="text-xl font-bold text-orange-400">{lifterB.metrics?.totalWork.toFixed(0) || "—"} J</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-slate-400">Total Power</div>
                      <div className="text-xl font-bold text-orange-400">{lifterB.metrics ? `${powerB.toFixed(0)} W` : "—"}</div>
                    </div>
                  </div>
                  <div className="text-xs text-slate-500 mt-1">{lifterB.metrics ? `${reps.toFixed(0)} reps in ${time.toFixed(1)}s` : "—"}</div>
                </div>
              </div>
            </div>
            ...
            {/* Comparison Summary */}
            {lifterB.metrics && (
              <div className="mt-3 p-3 bg-slate-800/50 rounded text-center space-y-1 text-xs text-slate-400">
                <div>
          ...
          {lifterB.metrics && (
            <div className="pt-4 border-t border-slate-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Lifter A Metabolic */}
                <div className="p-4 bg-gradient-to-br from-purple-900/40 to-slate-900/40 rounded-lg border border-purple-500/30">
                  <div className="text-xs font-semibold text-purple-300 mb-2">{lifterA.name}</div>
                  <div className="space-y-2">
                    <div>
                      <div className="text-xs text-slate-400">Rep Velocity</div>
                      <div className="text-lg font-bold text-purple-400">{velocityA.toFixed(2)} m/s</div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <div className="text-slate-400">Efficiency</div>
                        <div className="font-semibold text-white">{(efficiencyA * 100).toFixed(1)}%</div>
                      </div>
                      <div>
                        <div className="text-slate-400">Accel</div>
                        <div className="font-semibold text-white">{accelerationA.toFixed(2)} m/s²</div>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-purple-500/30">
                      <div className="text-xs text-slate-400">Total Energy</div>
                      <div className="text-xl font-bold text-purple-400">{metabolicCostA_kcal.toFixed(2)} kcal</div>
                      <div className="text-xs text-slate-500">({metabolicEnergyA.toFixed(0)} J)</div>
                    </div>
                  </div>
                </div>

                {/* Lifter B Metabolic */}
                <div className="p-4 bg-gradient-to-br from-indigo-900/40 to-slate-900/40 rounded-lg border border-indigo-500/30">
                  <div className="text-xs font-semibold text-indigo-300 mb-2">{lifterB.name}</div>
                  <div className="space-y-2">
                    <div>
                      <div className="text-xs text-slate-400">Rep Velocity</div>
                      <div className="text-lg font-bold text-indigo-400">{velocityB.toFixed(2)} m/s</div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <div className="text-slate-400">Efficiency</div>
                        <div className="font-semibold text-white">{(efficiencyB * 100).toFixed(1)}%</div>
                      </div>
                      <div>
                        <div className="text-slate-400">Accel</div>
                        <div className="font-semibold text-white">{accelerationB.toFixed(2)} m/s²</div>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-indigo-500/30">
                      <div className="text-xs text-slate-400">Total Energy</div>
                      <div className="text-xl font-bold text-indigo-400">{metabolicCostB_kcal.toFixed(2)} kcal</div>
                      <div className="text-xs text-slate-500">({metabolicEnergyB.toFixed(0)} J)</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-3 p-3 bg-slate-800/50 rounded text-center text-xs text-slate-400">
                {metabolicCostA_kcal > metabolicCostB_kcal ? (
                  <>
                    <span className="font-semibold text-purple-400">{lifterA.name}</span> expends{" "}
                  ...
                  <>
                    <span className="font-semibold text-indigo-400">{lifterB.name}</span> expends{" "}
      ...
      <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg shadow-sm border border-slate-700 p-6">
        {showEquivalentPerformance && (
          <div>
            <h4 className="text-lg font-semibold text-white mb-3">To Match {lifterA.name}&apos;s Performance:</h4>

            <div className="space-y-2 p-3 bg-gradient-to-r from-orange-900/20 to-orange-800/20 rounded-lg border border-orange-500/30">
              ...
              {lifterB.metrics && (
                <>
                  <p className="text-xs text-slate-300 leading-relaxed border-t border-orange-500/30 pt-2">
                    To complete the same {reps.toFixed(0)} reps in {time.toFixed(1)} seconds,{" "}
                    <span className="font-semibold text-orange-400">{lifterB.name}</span> would need to produce{" "}
                    {powerDifferencePercent > 0 ? (
                      <>
                        <span className="font-bold text-orange-400">{powerDifferencePercent.toFixed(1)}% less power</span>{" "}
                        ({powerB.toFixed(0)}W vs {powerA.toFixed(0)}W)
                      </>
                    ) : (
                      <>
                        <span className="font-bold text-orange-400">{Math.abs(powerDifferencePercent).toFixed(1)}% more power</span>{" "}
                        ({powerB.toFixed(0)}W vs {powerA.toFixed(0)}W)
                      </>
                    )}{" "}
      ...
      {/* Fun Data Nerd Stats */}
      <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg shadow-sm border border-slate-700 p-6">
        <h4 className="text-sm font-semibold text-slate-300 mb-3">📊 Deep Dive Metrics</h4>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
          {/* ROM Comparison */}
          <div className="p-2 bg-slate-800/50 rounded">
            <div className="text-xs text-slate-400">ROM Difference</div>
          ...
          {/* Effective Mass */}
          <div className="p-2 bg-slate-800/50 rounded">
            <div className="text-xs text-slate-400">Effective Mass</div>
          ...
          <div className="p-2 bg-slate-800/50 rounded">
            ...
            <div className="text-xs text-slate-500 mt-1">per rep</div>
          </div>

          {/* Peak Power Estimate */}
          <div className="p-2 bg-slate-800/50 rounded">
            <div className="text-xs text-slate-400">Displacement</div>
            <div className="font-semibold text-blue-400">
        ...
        <div className="mt-4 pt-4 border-t border-slate-700 grid grid-cols-2 gap-4 text-sm">
          <div className="relative">
            ...
            {activeTooltip === "displacement" && (
              <div className="absolute z-10 left-0 top-full mt-2 w-64 p-3 bg-slate-950 border border-slate-700 text-white text-xs rounded-lg shadow-lg">
                <div className="font-semibold mb-1 text-slate-200">Displacement Ratio = Range of Motion (ROM)</div>
          ...
          <div className="relative">
            ...
            {activeTooltip === "demand" && (
              <div className="absolute z-10 left-0 top-full mt-2 w-64 p-3 bg-slate-950 border border-slate-700 text-white text-xs rounded-lg shadow-lg">
                <div className="font-semibold mb-1 text-slate-200">Demand Ratio = Biomechanical Difficulty</div>
                <p className="mb-2 text-slate-300">
      ...
      {result.capacityAdjusted && (
        <div className="bg-gradient-to-br from-purple-900/40 to-indigo-900/40 rounded-lg shadow-sm p-6 border-2 border-purple-500/30">
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-lg font-semibold text-purple-200">
              Research-Adjusted Comparison
            </h3>
            <span className="px-2 py-1 bg-purple-900/50 text-purple-300 text-xs font-semibold rounded border border-purple-500/30">
              EXPERIMENTAL
            </span>
          </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg p-4 mb-4 border border-slate-700">
            <h4 className="font-semibold text-white mb-3">Load Capacity Factors</h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="text-slate-400">{lifterA.name}</div>
                <div className="font-semibold text-blue-400">
              ...
              <div>
                <div className="text-slate-400">{lifterB.name}</div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg p-4 border border-slate-700">
            <h4 className="font-semibold text-white mb-3">Adjusted Advantage</h4>
            ...
            <div className="text-sm">
              {result.capacityAdjusted.adjustedAdvantageDirection === "neutral" && (
                <div className="text-slate-300 p-2 bg-slate-800 rounded">
                  After capacity adjustment: Nearly identical difficulty
                </div>
              )}
              {result.capacityAdjusted.adjustedAdvantageDirection === "advantage_A" && (
                <div className="text-blue-300 p-2 bg-blue-900/20 rounded font-medium border border-blue-500/30">
              ...
              {result.capacityAdjusted.adjustedAdvantageDirection === "advantage_B" && (
                <div className="text-orange-300 p-2 bg-orange-900/20 rounded font-medium border border-orange-500/30">
